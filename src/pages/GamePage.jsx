...
const GamePage = () => {
  ...
  const loadtemperature = async () => {
    ...
    const secondTemp = await load(cities.find((item) => item.city === Needcity2));
    setTemperature2(secondTemp);
  };

  const changeCity = (isHigher) => {
    const conditionMet = isHigher ? temperature2 > temperature : temperature2 < temperature;
  ...
  return (
    <>
      ...
      <div className='fixed bottom-0 left-0 m-3 p-3 text-white text-lg md:text-3xl lg:text-4xl'>
        <p>Score: {score}</p>
      </div>
    </>
  );
